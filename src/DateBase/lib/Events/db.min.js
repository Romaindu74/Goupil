...
function open_db(User, path, level = 0) {
    const indexPath = `${globalThis.path}/lib/Json/${path}/index.json`;
    ...
    let decoded_string;
    try {
        const encoded_strings = index.files.map(file => fs.readFileSync(`${globalThis.path}/lib/Json/${path}${file}`, 'utf8'));
...
function save_db(path, data, level = 0) {
    ...
    if (!fs.existsSync(`${globalThis.path}/lib/Json/${path}/files/`)) {
        fs.mkdirSync(`${globalThis.path}/lib/Json/${path}/files/`);
    }
    ...
    hash.forEach((hashPart, i) => {
        ...
        fs.writeFileSync(`${globalThis.path}/lib/Json/${path}/files/${i}.json`, encoded_string, 'utf8');
    });
...
function ls(event, _) {
    ...
    const Path    = `${Clients.path}/lib/Json`;
...
function new_db(path, data = undefined, level = 1) {
    ...
    else if (fs.existsSync(`${globalThis.path}/lib/Json/${path}/index.json`))          return 1106;
    ...
    try {
        fs.mkdirSync(`${globalThis.path}/lib/Json/${path}/files/`, { recursive: true });
        fs.writeFileSync(`${globalThis.path}/lib/Json/${path}/index.json`, JSON.stringify(info, undefined, 4), 'utf8');
    } catch (e){
...
function del_db(path, level = 0) {
    if (`string` != typeof path)                           return 1101;
    else if (!fs.existsSync(`${globalThis.path}/lib/Json/${path}/index.json`)) return 1102;

    try {
        var index = JSON.parse(fs.readFileSync(`${globalThis.path}/lib/Json/${path}/index.json`, 'utf8'));
    } catch {
        return 1103;
    }
    ...
    fs.unlinkSync(`${globalThis.path}/lib/Json/${path}/index.json`);
    ...
    return { delete: remove(`${globalThis.path}/lib/Json/${path}/files/`) };
}
...
function exist_db(path) {
    ...
    if (!fs.existsSync(`${globalThis.path}/lib/Json/${path}/index.json`)) {
        return false;
    } else {
        return true;
    }
